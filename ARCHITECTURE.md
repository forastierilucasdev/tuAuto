...
- **Rate limiting**: `lib/rate-limit.ts` usa Redis (`@upstash/ratelimit` + `@upstash/redis`, sliding window) si `UPSTASH_REDIS_REST_URL`/`UPSTASH_REDIS_REST_TOKEN` están configuradas en el entorno — preciso con cualquier cantidad de instancias serverless. Sin esas variables, cae a un Map in-memory por proceso (solo sirve para desarrollo local o un único proceso). Aplicado por email/usuario en login (`authorize()` de Auth.js y `loginAction`), registro, recuperar contraseña y cambiar contraseña.
