...
## Fase 84 — Panel de detalle privado (estado/fechas/vistas), Activas en Resumen, y limpieza de duplicación (solicitado por el usuario)
- [x] **Contador "Activas" faltante en "Administrador de anuncios"** — causa de que los totales entre ese panel y "Mis publicaciones" no coincidieran (el Resumen tenía Destacadas/Reservadas/Inactivas/Vendidas pero no Activas). Se agregó, misma fuente de datos (`getOwnerListingGroups`) que ya usan las pestañas de "Mis publicaciones" — cero riesgo de que vuelvan a divergir
- [x] `/catalogo/[slug]`: "Volver" dejó de ser un `href` fijo a `/catalogo` — ahora depende de quién mira (dueño → `/dashboard/publicaciones`, Superadmin → `/admin/publicaciones`, cualquier otro → `/catalogo`). Antes, el dueño entrando desde "Mis publicaciones" terminaba en el catálogo general sin poder volver ni con "Volver" ni con el botón atrás del navegador (el "Volver" viejo agregaba su propia entrada al historial, tapando el camino de vuelta)
- [x] `getListingBySlug()` gana un bypass para Superadmin (mismo criterio que ya tiene el dueño) — antes un Superadmin no podía abrir `/catalogo/[slug]` de una publicación ajena que no fuera pública (pausada, pendiente, etc.), le daba 404
- [x] Panel privado debajo del título (visible solo para el dueño y el Superadmin, nunca para otro visitante): estado real (con la misma etiqueta/color que "Mis publicaciones"), ícono de info en "Pendiente de aprobación" que abre un modal con el mismo texto literal del wizard, fecha de creación, "Visible en catálogo hasta: [fecha]", y vistas acumuladas
- [x] `generateMetadata()` de esa misma página también resuelve la sesión — antes el título de la pestaña decía "Publicación no encontrada" para el propio dueño viendo su publicación inactiva, aunque la página cargaba bien
- [x] **Limpieza de duplicación** (a pedido explícito, en vez de dejarlo para la auditoría futura): `PENDING_APPROVAL_MESSAGE` centralizado en `constants.ts` — antes el mismo texto literal de 3 líneas estaba copiado en `VehicleNotListedModal`, `LocalityNotListedModal`, `ListingForm` (banner de confirmación) y una versión reescrita en `PublishedListingModal`; ahora los 5 usan la misma constante. `LISTING_STATUS_LABEL`/`LISTING_STATUS_BADGE_VARIANT` centralizados igual — antes copiados casi textual en `OwnerListingCard.tsx` y `admin/publicaciones/page.tsx`, ahora una sola fuente de verdad para las 8 etiquetas de estado
- [x] Nuevo `InfoModalButton` (`components/ui/`) — ícono de información genérico y reutilizable que abre un modal con un mensaje, para cualquier aclaración corta futura (no solo "Pendiente de aprobación")
- [x] `getEffectiveStatus()` pasa de privada a exportada en `server/data/listings.ts`, para poder calcular el estado real en la página de detalle sin reimplementar la lógica
- [x] `tsc --noEmit`, `eslint`, `npm run build` limpios
- [x] Servidor de desarrollo sin errores de compilación; smoke test de `/catalogo/[slug]` real sin excepciones
- [ ] Prueba manual en navegador (no se pudo probar de forma interactiva desde acá)

## Fase 83 — Ronda de bugs reportados en producción + confirmación centralizada + bloqueo de identidad verificada (solicitado por el usuario)
Lista de 10+ reportes de una sesión de prueba real en `motores-ya-seven.vercel.app`, más pedidos de seguimiento en la misma ronda.
- [x] **CSP sin `blob:` en `img-src`** — causa raíz de varios reportes a la vez: las vistas previas locales (`URL.createObjectURL`) de fotos de auto, avatar y logo de agencia se bloqueaban en producción ("Loading the image 'blob:...' violates CSP"). Esto explicaba: la vista previa rota al elegir una foto, el picker de posición "solo funcionando después de guardado" (la imagen nunca cargaba antes), y la estrella de foto destacada "sin efecto" (sí reordenaba, pero no se veía nada roto)
- [x] Wizard: orden de campos corregido a Tipo → Marca → Modelo → Versión → Año (antes Tipo/Año/Marca/Modelo)
- [x] Wizard: la barra de botones (Atrás/Siguiente/Publicar) pasa a `sticky bottom-0` con fondo propio, para que nunca quede ambigua/mezclada con el contenido al hacer scroll
- [x] `RegisterForm.tsx`: `try/catch/finally` faltante — una excepción no controlada dejaba el botón trabado en "Creando cuenta..." para siempre sin ningún mensaje (parecía que la cuenta nunca se creaba); + modal "Cuenta creada exitosamente" con tilde
- [x] `PublishedListingModal.tsx`: mostraba "Tu anuncio fue publicado con éxito / Ya está visible en el catálogo" incluso para una publicación que quedó `PENDIENTE_APROBACION` — ahora es status-aware, con el mensaje correcto de "quedó pendiente de revisión" cuando corresponde
- [x] **Bug real encontrado**: `validateAndActivateListingAction` no revalidaba `/dashboard/publicaciones`/`/dashboard/anuncios` — el cupo SÍ se descontaba bien en la base (ya verificado en la Fase 80), pero el panel del dueño podía seguir mostrando el contador viejo en caché
- [x] Confirmación de email/DNI duplicados al registrarse: ya estaba implementado (`emailExists`/`dniExists` en `registerAction`), se confirmó sin cambios de código
- [x] Selector Particular/Agencia/Concesionaria (`grid-cols-3`): "Concesionaria" (palabra sin espacios) forzaba esa columna a ensancharse más allá de su 1/3 y el botón quedaba fuera del contenedor en pantallas angostas — `min-w-0 truncate` + tamaño de fuente responsive, corregido en `AccountTypeForm.tsx` y `RegisterForm.tsx`
- [x] "Sitio web" ya era opcional en código (schema y HTML) — se le agregó "(opcional)" al label para que quede explícito
- [x] **Identidad verificada bloquea datos sensibles**: una vez que `isVerified` es `true`, apellido/nombre/DNI (`ProfileForm.tsx`) y razón social/CUIT (`AccountTypeForm.tsx`) quedan de solo lectura — `readOnly`, no `disabled` (un input deshabilitado no manda su valor en el `FormData` y rompería la validación del resto del formulario). Reforzado server-side en `updateProfileAction` (nunca se confía solo en el `readOnly` del cliente). Excepción cuidada: si pasa de Particular a Agencia por primera vez, razón social/CUIT NO se bloquean (todavía no hay nada que proteger)
- [x] **Confirmación centralizada**: nuevo `SuccessModalBody` (`components/ui/`) — tilde + mensaje + botón, reutilizado en vez de duplicado. Aplicado a `AdminConfirmButton`/`ReasonConfirmModal`/`SuspendActionModal` (cubre automáticamente Marcar Activa/Vendida, Pausar, Suspender, Dar de baja, Validar datos, y todos los toggles de baja/reactivar de Marca/Modelo/Versión/Provincia/Localidad/Tipo de vehículo/Plan), `ProfileForm`, `RegisterForm`, `ListingEditForm`, `TaxonomyRequestRowActions`, `LocalityRequestRowActions` — en todos, la acción ya no cierra el modal sola: pasa a una pantalla de "¡Listo!" con cruz para cerrar, y `onSuccess`/`router.refresh()` se dispara recién al cerrarla
- [x] Patrón "ajustar estado durante el render" (no `setState` dentro de un efecto, evita cascading renders — lo marcó el linter) para que el modal de éxito vuelva a aparecer aunque dos guardados seguidos sean ambos exitosos
- [x] Logo real (`/logo-v3.svg`, mismo asset del sitio público) en el header del panel admin — antes solo decía "Motoresya Admin" en texto plano
- [x] `SupportForm.tsx`: cuando se llega desde "Reportar error" de una publicación puntual (`OwnerListingCard` → `/dashboard/soporte?listingId=...`, con un ID real, no tecleado), el campo ID de la publicación pasa a ser obligatorio y de solo lectura (`readOnly`, no `disabled`, para que el valor se siga enviando) — es el dato necesario para poder revisar el error. Si se entra a Soporte sin venir de una publicación puntual, sigue siendo texto libre opcional como antes
- [x] `tsc --noEmit`, `eslint`, `npm run build` limpios
- [x] Servidor de desarrollo sin errores de compilación en las rutas tocadas
- [ ] Prueba manual en navegador de cada punto (no se pudo probar de forma interactiva desde acá — se corrigió a partir de capturas de pantalla y razonamiento sobre el código, no reproducción en vivo)
- [ ] Auditoría futura de código repetido/centralizable (acordada con el usuario como actividad aparte, ver memoria del asistente) — el candidato más claro son los ~7 modales de alta/edición de catálogo (`BrandFormModal`, `ModelFormModal`, `VersionFormModal`, `ProvinceFormModal`, `LocalityFormModal`, `VehicleTypeFormModal`, `EditPlanModal`), que repiten casi textual el mismo esqueleto

## Fase 82 — Reposicionar foto de perfil y foto de portada de agencia (solicitado por el usuario)
Antes se centraban/recortaban automáticamente (`object-cover` fijo); ahora el usuario puede arrastrar la imagen para elegir qué parte queda visible, sin cambiar el zoom.
- [x] `User.avatarPositionX/Y` y `AgencyProfile.logoPositionX/Y` (migración aditiva, `Int @default(50)` cada uno — 50/50 = centrado, mismo comportamiento que antes para las filas existentes, sin backfill manual)
- [x] `ImagePositionPicker.tsx` (nuevo, `components/ui/`): widget reutilizable de arrastrar-para-centrar — la imagen siempre cubre el marco (`object-fit: cover`, nunca deja huecos), arrastrar solo mueve el `object-position`; funciona con mouse y touch (Pointer Events + `touch-none`)
- [x] `ProfileForm.tsx` (foto de perfil) y `AccountTypeForm.tsx` (foto de portada de agencia/concesionaria) reemplazan el círculo/rectángulo estático por el picker, con un botón de cámara superpuesto para elegir archivo; al elegir una foto nueva la posición vuelve a 50/50 (la vieja posición era para la imagen anterior)
- [x] `avatarPositionX/Y`/`logoPositionX/Y` viajan fuera del schema de zod (mismo criterio que `avatarUrl`/`logoUrl`: se leen directo del `FormData` en `profile.actions.ts`, clampeados 0-100) — cada pantalla solo manda la posición que le corresponde, la otra queda sin tocar
- [x] `UserAvatar.tsx`, `AgencyCard.tsx` y el encabezado de `/concesionarias/[id]` aplican la posición guardada vía `style={{ objectPosition }}` — se extendió el `select` de `getFullProfile`/`getListingBySlug`/`loadAgencies` para traer los campos nuevos
- [x] `ERRORES.md`: se marca como resuelta la limitación "Foto de perfil sin editor de recorte" (ahora cubre también la foto de agencia)
- [x] `tsc --noEmit`, `eslint`, `npm run build` limpios
- [x] Verificado con script desechable contra la base real: default 50/50 en una fila ya existente, guardar y leer una posición nueva en `User`, y lo mismo en un `AgencyProfile` (creado y borrado de prueba) — revertido
- [ ] Prueba manual en navegador del arrastre en sí (drag-and-drop) — no se pudo probar de forma interactiva desde acá, solo se verificó que compila y persiste correctamente

## Fase 81 — Catálogo administrable: Fase 10/10, limpieza y documentación (solicitado por el usuario)
Cierra la iniciativa de 10 fases completa — sin código funcional nuevo, solo verificación final y documentación.
- [x] Auditoría revisada: las 8 tablas nuevas de esta iniciativa (`VehicleTypeCatalog`, `Brand`, `Model`, `Version`, `Province`, `Locality`, `TaxonomyRequest`, `LocalityRequest`) ya estaban completas en `AdminAuditTargetTable`/`resolveAuditTargets()`/`admin-audit-labels.ts` — se fueron extendiendo fase a fase, no quedó ninguna pendiente
- [x] Sidebar revisado: `AdminSidebarNav.tsx` y el `MOBILE_NAV` duplicado de `AdminHeader.tsx` ya tenían los links a "Vehículos"/"Ubicación" (agregados en Fases 2/5) — las 2 colas de pendientes (`/admin/vehiculos/pendientes`, `/admin/ubicacion/pendientes`) se acceden desde el link "Tareas pendientes" dentro de cada sección, mismo criterio que `/admin/publicaciones/[id]/editar` (no está en el sidebar tampoco)
- [x] `ERRORES.md`: reemplazada la limitación desactualizada ("sin gestión del catálogo de marcas/modelos", ya resuelta) por la limitación real que queda — el `<select>` de Tipo del wizard/filtros todavía lee `VEHICLE_TYPES` fijo, no `VehicleTypeCatalog` (documentado como fuera de alcance de las 10 fases a propósito)
- [x] `ARCHITECTURE.md` §7.2.7: estado final "completo, 10/10", con un resumen de qué quedó fuera a propósito
- [x] `tsc --noEmit` y `eslint` corridos sobre **todo** `src/` (no solo los archivos tocados en la última fase) — limpios
- [ ] Prueba manual en navegador de punta a punta: publicar con vehículo/localidad "no listados" → aprobar ambas solicitudes en las colas de pendientes → "Validar datos" → confirmar que se publica y descuenta el crédito

## Fase 80 — Catálogo administrable: Fase 9/10, botón "Validar datos" en /admin/publicaciones/[id] (solicitado por el usuario)
El segundo paso del flujo de aprobación en dos pasos — cierra el círculo: aprobar la solicitud de catálogo (Fase 8) resuelve el dato, "Validar datos" (acá) recién publica y descuenta el crédito.
- [x] `validateAndActivateListingAction` (nuevo, `server/actions/admin/listings.actions.ts`): envuelve `validateAndActivateListing` (ya construida en la Fase 6) con `requireAdminPermission`, auditoría (`listing.setStatus`, mismo action que "Marcar Activa") y revalidación; traduce `QuotaExceededError`/`AccountSuspendedError`/`TaxonomyPendingError` a mensajes legibles
- [x] `ListingStatusActions.tsx`: botón "Validar datos" nuevo, visible solo si `status === "PENDIENTE_APROBACION"`, **deshabilitado mientras `pendingTaxonomyRequestId`/`pendingLocalityRequestId` sigan sin resolver** — el aviso de la Fase 6 ya le indica al admin ir a "Tareas pendientes" primero
- [x] `tsc --noEmit`, `eslint`, `npm run build` limpios
- [x] Verificado con script desechable contra la base real, caso completo: listing con AMBAS referencias sin resolver → "Validar datos" rechazado; se aprueba la `TaxonomyRequest` → sigue rechazado (falta la `LocalityRequest`); se aprueba la `LocalityRequest` → recién ahí "Validar datos" pasa el listing a `ACTIVE`, setea `publishedAt` y descuenta 1 cupo (y suma 1 a `activationCount`) — nunca antes; reintentar "Validar datos" sobre un listing ya `ACTIVE` se rechaza — revertido todo
- [x] Servidor de desarrollo sin errores de compilación
- [ ] Prueba manual en navegador

## Fase 79 — Catálogo administrable: Fase 8/10, colas de "Tareas pendientes" en admin (solicitado por el usuario)
El precedente exacto calcado, como estaba previsto en el plan, es `/admin/identidad`: cola con "Aprobar" o "Dejar observación", nunca un rechazo duro.
- [x] `/admin/vehiculos/pendientes` y `/admin/ubicacion/pendientes` (nuevas): listan `TaxonomyRequest`/`LocalityRequest` en estado `PENDING`, con cuántas publicaciones tiene vinculadas cada una — una sola aprobación puede destrabar varias a la vez, de distintos usuarios
- [x] "Aprobar" abre un mini-form (`TaxonomyRequestRowActions`/`LocalityRequestRowActions`) que **confirma o corrige** el nombre final tecleado por el usuario antes de crear las filas reales — verificado explícitamente que si el admin edita el nombre, se crea con el nombre EDITADO, no el original
- [x] "Guardar observación": mismo criterio que Identidad, la solicitud sigue `PENDING`, la nota queda para la próxima revisión — nunca hay un botón de "rechazar"
- [x] `server/actions/admin/taxonomy-requests.actions.ts`/`locality-requests.actions.ts` (nuevos): envuelven `approveTaxonomyRequest`/`approveLocalityRequest`/`addTaxonomyRequestNote`/`addLocalityRequestNote` (ya construidas en la Fase 6) con `requireAdminPermission`, auditoría y revalidación
- [x] Auditoría extendida: `TaxonomyRequest`/`LocalityRequest` sumadas a `AdminAuditTargetTable`, `resolveAuditTargets()`, `ACTION_LABEL`/`TARGET_TABLE_LABEL`/`TARGET_TABLES`
- [x] `/admin/vehiculos` y `/admin/ubicacion` ganan un link "Tareas pendientes" con badge de cantidad (solo si hay alguna pendiente)
- [x] `tsc --noEmit`, `eslint`, `npm run build` limpios
- [x] Verificado con script desechable contra la base real: la solicitud aparece en la cola con el conteo correcto de publicaciones vinculadas, agregar observación no cambia el status, aprobar con un nombre editado por el admin crea la fila real con ESE nombre (no el tecleado original), aprobar destraba el listing sin cambiarle el status (sigue `PENDIENTE_APROBACION`, falta la Fase 9), y la solicitud sale de la cola de pendientes — revertido
- [x] Servidor de desarrollo: ambas rutas nuevas redirigen a `/login` sin sesión, sin errores de compilación
- [ ] Prueba manual en navegador

## Fase 78 — Catálogo administrable: Fase 7/10, modales del wizard "no está en la lista" (solicitado por el usuario)
La parte visible para el usuario del flujo de aprobación en dos pasos — conecta el wizard con la capa de datos de la Fase 6.
- [x] `VehicleNotListedModal.tsx`/`LocalityNotListedModal.tsx` (nuevos, `components/forms/`): autocontenidos (Tipo/Año propios en el de vehículo, Provincia propia en el de localidad — nunca reusan el estado del paso principal), con el texto explicativo literal pedido por el usuario: *"Los datos ingresados serán validados por el administrador, una vez aprobados, la publicación será visible en el catálogo del sitio, mientras tanto estará inactiva y no te descontará publicaciones disponibles."*
- [x] **"Guardar" del modal NO llama a ningún Server Action** — solo junta los datos en memoria (`pendingVehicle`/`pendingLocality` en `ListingForm`). El `TaxonomyRequest`/`LocalityRequest` real recién se crea dentro de `createListing` si el usuario efectivamente termina publicando, así no queda una solicitud huérfana si abandona el wizard a mitad de camino
- [x] Mientras hay datos pendientes, el bloque Tipo/Marca/Modelo/Versión/Año (o Provincia/Localidad) se reemplaza por un resumen fijo con botón "Quitar y elegir de la lista" para volver atrás
- [x] `createListingSchema` (`lib/validations/listing.ts`) con 4 `.refine()`: brandSlug+modelSlug XOR pendingBrandName (nunca ambos, nunca ninguno); pendingBrandName exige modelo+versión también cargados; localitySlug XOR pendingLocalityName (nunca ambos); pendingLocalityName exige provinceSlug real
- [x] Paso "revisar" del wizard: aviso *"Esta publicación quedará pendiente de revisión"* cuando hay datos pendientes, con el resumen de Vehículo/Ubicación mostrando el texto tecleado en vez de los slugs resueltos
- [x] Modal de confirmación final: título/texto/botón cambian a la variante de "enviar a revisión" (mismo texto literal pedido) cuando corresponde
- [x] `tsc --noEmit`, `eslint`, `npm run build` limpios
- [x] Verificado con script desechable: las 8 reglas de `.refine()` del schema probadas una por una (válido/rechazado en cada combinación), y el camino "pendiente" de `createListing()` contra la base real — listing queda en `PENDIENTE_APROBACION` sin `brandId`/`modelId`, título compuesto con el texto tecleado, no consume cupo, y 2 listings con el mismo pedido reusan la MISMA `TaxonomyRequest`/`LocalityRequest` (dedupe real, no simulado) — revertido
- [x] Servidor de desarrollo sin errores de compilación
- [ ] Prueba manual en navegador (publicar un vehículo/localidad "no listados" de punta a punta)

## Fase 77 — Catálogo administrable: Fase 6/10, estado PENDIENTE_APROBACION + TaxonomyRequest/LocalityRequest + validateAndActivateListing (solicitado por el usuario)
Capa de datos, sin UI todavía (las colas de admin son la Fase 8, los modales del wizard la Fase 7) — verificado de punta a punta con scripts contra la base real.
- [x] `ListingStatus.PENDIENTE_APROBACION` (migración aislada de `ALTER TYPE ... ADD VALUE`, en su propio `db execute`, nunca junto con el resto del schema en la misma transacción — mismo cuidado que ya se tuvo con SUSPENDIDA)
- [x] `TaxonomyRequest`/`LocalityRequest` (tablas nuevas): reusan `VerificationStatus` (nunca `REJECTED` acá — "aprobar" o "dejar pendiente con nota", nunca un rechazo duro, mismo criterio que Identidad). `dedupeKey` único (vía `slugify()`) evita que dos usuarios pidiendo lo mismo generen dos filas — la segunda solicitud simplemente se linkea a la ya pendiente
- [x] `Listing.brandId`/`.modelId` pasan a nullable (antes `NOT NULL`) + `pendingBrandName`/`pendingModelName`/`pendingVersionName`/`pendingTaxonomyRequestId` y `pendingLocalityName`/`pendingLocalityRequestId` — solo tienen valor mientras el listing está pendiente; la Provincia siempre es real (lista fija de 24), solo la Localidad puede quedar pendiente
- [x] Ripple de la nulabilidad revisado y corregido en los ~5 puntos reales que asumían `.brand.name`/`.model.name` no nulos (detalle público, ambos wizards de edición, editor básico de admin) — las páginas de edición con wizard completo ahora muestran un aviso en vez de la cascada cuando el listing está pendiente (no tiene slugs reales todavía contra los que armar Marca→Modelo→Versión)
- [x] `createListing()` extendido: acepta `pendingBrandName`/`pendingModelName`/`pendingVersionName` (mutuamente excluyente con `brandSlug`/`modelSlug`/`versionSlug`) y `pendingLocalityName` (mutuamente excluyente con `localitySlug`, requiere `provinceSlug` real) — hace *find-or-create* de la solicitud por `dedupeKey`, guarda el listing en `PENDIENTE_APROBACION` sin FKs reales de marca/modelo, **sin consumir cupo ni bloquear por suspensión** (mismo trato que un borrador)
- [x] `buildActivationEffect`/`assertAvailable`/`assertNotSuspended` (antes privadas de `server/data/listings.ts`) pasan a exportadas para que `validateAndActivateListing` las reuse sin reimplementar la lógica de cupo por quinta vez
- [x] `validateAndActivateListing()` (nuevo, `server/data/admin/listings.ts`) — el futuro botón "Validar datos" (Fase 9): rechaza si el listing no está pendiente o si todavía tiene `pendingTaxonomyRequestId`/`pendingLocalityRequestId` sin resolver (`TaxonomyPendingError`), si no pasa a `ACTIVE` y recién ahí descuenta 1 cupo — **dos pasos separados de aprobar la solicitud de catálogo**, por decisión explícita del usuario
- [x] `approveTaxonomyRequest()`/`approveLocalityRequest()` (nuevos, `server/data/admin/taxonomy-requests.ts`/`locality-requests.ts`): crean (o reusan por nombre) las filas reales de Brand/Model/Version o Locality, y actualizan con `updateMany` TODOS los listings vinculados a esa solicitud de una — una aprobación puede destrabar varios listings de distintos usuarios a la vez. No tocan `status` (eso es "Validar datos", Fase 9)
- [x] `tsc --noEmit`, `eslint`, `npm run build` limpios
- [x] Verificado con script desechable contra la base real: 2 listings pidiendo lo mismo → 1 sola `TaxonomyRequest`/`LocalityRequest` (dedupe real, no simulado); crear pendiente no consume cupo; "Validar datos" antes de aprobar las solicitudes se rechaza; aprobar cada solicitud destraba los 2 listings vinculados sin tocar su `status`; "Validar datos" recién ahí pasa a ACTIVE y descuenta 1 cupo — revertido todo (listings, Brand/Model/Version/Locality de prueba borrados, cupo del usuario restaurado; las solicitudes ya aprobadas quedan como historial, mismo criterio que `VerificationRequest`)
- [x] Servidor de desarrollo sin errores de compilación tras el cambio
- [ ] Prueba manual en navegador (sin sentido todavía — no hay UI que dispare este camino hasta la Fase 7/8/9)

## Fase 76 — Catálogo administrable: Fase 5/10, Province/Locality administrables + wizard en cascada (solicitado por el usuario)
Migración puramente aditiva (decisión explícita del usuario): las publicaciones viejas conservan su texto libre de provincia/localidad sin intentar vincularlas retroactivamente.
- [x] `Province`/`Locality` (tablas nuevas): `Province.name`/`.slug` únicos, `Locality` con `@@unique([provinceId, name])` — mismo criterio `isActive`/`sortOrder` que el resto del catálogo administrable
- [x] `Listing.provinceId`/`.localityId` (FK nullables, `ON DELETE SET NULL`) — `Listing.province`/`.city` (texto libre histórico) se mantienen para siempre, sincronizados con `provinceRef.name`/`localityRef.name` cuando el wizard resuelve una ubicación real
- [x] `server/data/admin/locations.ts` + `server/actions/admin/locations.actions.ts` (nuevos): CRUD de provincias/localidades, `seedProvincesFromConstant()` (idempotente, siembra las 24 provincias fijas de `constants.ts`) y `createLocalitiesBulk()` (alta masiva, una localidad por línea de textarea — sin parser de CSV en el proyecto)
- [x] `/admin/ubicacion` (tabla de provincias + botón "Cargar provincias registradas" + modal alta/edición) y `/admin/ubicacion/[id]` (localidades de esa provincia: alta una por una, importación en lote, editar, dar de baja/reactivar) — mismos componentes reusados de la Fase 75 (`EntityActiveToggle`, `AdminConfirmButton`)
- [x] `resolveLocationPatch()` (nuevo, exportado de `server/data/listings.ts`, reusado por `updateOwnedListing` y `adminUpdateListing`): mismo criterio que `resolveVersionPatch` mismo aplicado a 2 niveles independientes (provincia y localidad) — no pisa el texto legado si nunca hubo una fila real resuelta y sigue sin elegirse una
- [x] Wizard: Provincia/Localidad pasan de un `<select>` de lista fija + `<input>` de texto libre a una cascada real Provincia→Localidad (mismo patrón que Marca→Modelo→Versión), con el mismo patrón de "opción huérfana" + párrafo informativo para publicaciones con texto legado
- [x] `ListingEditForm.tsx` (editor básico de admin): mismo cambio que en la Fase 75 con Versión — de `<input name="province">`/`<input name="city">` de texto libre a `<select>` en cascada, ahora con estado propio (`useLocationTaxonomy`) porque a diferencia de Versión (fija al modelo del listing) la Provincia sí puede cambiar libremente
- [x] **Decisión de diseño**: el filtro del catálogo público (`CatalogFilters.tsx`, `buildWhere()`) sigue comparando contra el texto legado (`province`/`city`) sin cambios — como ese texto queda sincronizado con el nombre real de `Province`/`Locality` cuando el wizard resuelve una ubicación, el filtro sigue funcionando igual para publicaciones viejas y nuevas sin tocarlo, evitando ampliar el alcance de esta fase
- [x] Auditoría extendida: `Province`/`Locality` sumadas a `AdminAuditTargetTable`, `resolveAuditTargets()`, `ACTION_LABEL`/`TARGET_TABLE_LABEL`/`TARGET_TABLES`
- [x] Sidebar admin actualizado (`AdminSidebarNav.tsx` y el `MOBILE_NAV` duplicado de `AdminHeader.tsx`)
- [x] `tsc --noEmit`, `eslint`, `npm run build` limpios
- [x] Verificado con script desechable contra la base real: seed idempotente de las 24 provincias (verificado que re-sembrar no duplica), dedupe de Locality, alta masiva (3 nuevas + 1 duplicada omitida), toggle `isActive`, los 3 casos de `resolveLocationPatch()`, ciclo completo de un listing real con ubicación vinculada (crear → deseleccionar → confirmar limpieza) — revertido, **excepto la siembra de las 24 provincias reales, que se dejó aplicada a propósito** para que el wizard no muestre un desplegable vacío en producción
- [x] Servidor de desarrollo: `/admin/ubicacion` y `/admin/ubicacion/[id]` redirigen a `/login` sin sesión, sin errores de compilación; `/catalogo` y `/` siguen en 200
- [ ] Prueba manual en navegador

## Fase 75 — Catálogo administrable: Fase 4/10, CRUD Marca/Modelo + tabla Version + wizard en cascada (solicitado por el usuario)
- [x] `Brand.isActive`/`Model.isActive` (migración aditiva, `NOT NULL DEFAULT true`) — dar de baja no borra, solo deja de ofrecerse como opción nueva; no afecta publicaciones existentes
- [x] Tabla `Version` nueva (`modelId`, `name`, `slug`, `isActive`, `@@unique([modelId, name])`) + `Listing.versionId` nullable con FK `ON DELETE SET NULL` — `Listing.version` (texto libre histórico) se mantiene para siempre, nunca se borra
- [x] `server/data/admin/vehicles.ts` (nuevo): CRUD completo de Brand/Model/Version — listar, crear, editar, dar de baja/reactivar — mismo patrón que `VehicleTypeCatalog` (Fase 73)
- [x] `server/actions/admin/vehicles.actions.ts` (nuevo): Server Actions con `requireAdminPermission("vehiculos","edit")` + auditoría + revalidación del catálogo público
- [x] `EntityActiveToggle.tsx` (nuevo, genérico): reemplaza lo que hubiera sido un 3er/4to casi-duplicado de `PlanActiveToggle`/`VehicleTypeActiveToggle` — esos dos quedan sin tocar, no se refactorizan retroactivamente
- [x] `BrandFormModal.tsx`/`ModelFormModal.tsx`/`VersionFormModal.tsx` (nuevos) — mismo patrón `mode: "create"|"edit"` que `EditPlanModal`/`VehicleTypeFormModal`; `VersionFormModal` agrega cascada Marca→Modelo (filtro client-side, sin fetch extra)
- [x] `/admin/vehiculos` reescrito con 4 pestañas por query-param (`?tab=tipos|marcas|modelos|versiones`) — sin componente de tabs nuevo, mismo criterio "arrays planos" que el resto del admin
- [x] Wizard: Versión pasa de texto libre a desplegable en cascada Tipo→Marca→Modelo→Versión→Año (`useVehicleTaxonomy` gana un 4º nivel); `versionSlugSchema` reemplaza a la validación de texto libre
- [x] `resolveVersionPatch()` (nuevo, exportado de `server/data/listings.ts`, reusado por `updateOwnedListing` y `adminUpdateListing`): distingue 3 casos para no pisar texto legado — (a) nunca tuvo versión resuelta y no se elige ninguna → no toca nada; (b) tenía una versión resuelta y se deselecciona → la limpia; (c) se elige una versión del desplegable → resuelve nombre + id y sincroniza `version` (texto legado) con el nombre real
- [x] Patrón de "opción huérfana" (ya usado en Marca/Modelo) extendido a Versión en modo edición; para versiones puramente de texto libre (sin fila `Version` nunca creada) se agrega en su lugar un párrafo informativo con el texto histórico, en vez de una opción falsa
- [x] `ListingEditForm.tsx` (editor básico de admin, distinto del wizard completo): el `<Input name="version">` de texto libre — que había quedado roto en silencio por el cambio de schema `version`→`versionSlug` — se reemplaza por un `<Select name="versionSlug">` con las versiones del modelo actual de la publicación, mismo patrón de opción huérfana + párrafo informativo que el wizard
- [x] Auditoría extendida: `Brand`/`Model`/`Version` sumadas a `AdminAuditTargetTable`, `resolveAuditTargets()`, `ACTION_LABEL`/`TARGET_TABLE_LABEL`/`TARGET_TABLES`
- [x] `tsc --noEmit`, `eslint`, `npm run build` limpios
- [x] Verificado con script desechable contra la base real: dedupe de Brand/Model/Version (slug/nombre duplicado rechazado), toggle `isActive`, los 3 casos de `resolveVersionPatch()`, ciclo completo de un listing real con versión vinculada (crear con versión → deseleccionar → confirmar que limpia `version`/`versionId`) — revertido
- [x] Servidor de desarrollo: `/admin/vehiculos` (las 4 pestañas) redirige a `/login` sin sesión, sin errores de compilación; `/catalogo` sigue en 200
- [ ] Prueba manual en navegador

## Fase 74 — Catálogo administrable: Fase 3/10, migrar Model/Listing del enum VehicleType a vehicleTypeId (solicitado por el usuario)
La fase de mayor riesgo del proyecto — convierte el enum fijo de Postgres en una FK real contra `VehicleTypeCatalog`. Verificado antes de aplicar (0 filas sin match) y después (mismos totales, datos intactos).
- [x] Migración con backfill manual en 8 pasos: columna nueva nullable → `UPDATE` por `code` → `NOT NULL` → recrear índices/constraint con los nombres exactos que Prisma espera → drop de columnas/índices viejos → `DROP TYPE` del enum
- [x] `Model`/`Listing.vehicleType` pasa de enum a relación (`vehicleTypeId` + relación `vehicleType → VehicleTypeCatalog`) — mismo criterio que ya usan `brandId`/`modelId`
- [x] `Model.@@unique([brandId, vehicleType, name])` → `@@unique([brandId, vehicleTypeId, name])`; mismo cambio en el `@@index` de `Listing`
- [x] Filtros por tipo de vehículo (catálogo público, admin, cascada de taxonomía) pasan de comparar el valor crudo a un filtro de relación por `code` (`where.vehicleType = { code }`) — mismo patrón que `brandSlug`/`modelSlug`, nunca se filtra por el `id` interno
- [x] Actualizados los ~14 archivos que dependían del tipo `VehicleType` de Prisma: `useVehicleTaxonomy.ts`, `taxonomy.actions.ts`, `taxonomy.ts`, `listings.ts` (`createListing`/`buildWhere`/`toVehicleCardData`/`getListingBySlug`/`getOwnedListingForEdit`), `admin/listings.ts`, `ListingForm.tsx`, `CatalogFilters.tsx`, `HeroSearch.tsx`, páginas de catálogo/publicaciones/editar, `prisma/seed.ts` (resuelve `code→id` vía lookup en vez de confiar en el chequeo de TypeScript que daba el enum)
- [x] `VEHICLE_TYPES`/`mileageUnitFor`/`usesTransmission` en `constants.ts` **sin cambios** — ya recibían `string`, no el enum; alcance de esta fase fue solo la capa de datos, no todavía "el wizard lee tipos nuevos de la base" (pendiente para una fase futura, ver nota)
- [x] `tsc --noEmit`, `eslint`, `npm run build` limpios
- [x] Verificado exhaustivamente contra la base real: conteo de filas sin match antes de aplicar (0 de 60 Model + 38 Listing), migración aplicada, mismos totales después, cascada de taxonomía + filtro de catálogo + ciclo completo de `createListing` con datos reales (creado, verificado con `include`, borrado) — y servidor de desarrollo con requests reales a home/catálogo (con filtro `tipo=`)/detalle/`/admin/vehiculos`
- [ ] Prueba manual en navegador

**Nota para una fase futura**: el wizard/filtros todavía leen `VEHICLE_TYPES` (constante hardcodeada en `constants.ts`), no `VehicleTypeCatalog` — un tipo nuevo creado desde `/admin/vehiculos` (Fase 2) no aparece todavía como opción para publicar/filtrar. Falta wire completo (leer tipos activos desde la base) para que el CRUD de Tipos tenga efecto de punta a punta.

## Fase 73 — Catálogo administrable: Fase 2/10, VehicleTypeCatalog + CRUD admin de Tipos (solicitado por el usuario)
- [x] `VehicleTypeCatalog` (tabla nueva, migración aditiva): `code`/`label`/`labelPlural`/`icon`/`mileageUnit`/`usesTransmission`/`isActive`/`sortOrder` — sembrada con los 7 valores actuales del enum `VehicleType` (mismos datos que `VEHICLE_TYPES` en `constants.ts`)
- [x] **Todavía en paralelo con el enum**: `Model`/`Listing` siguen usando `VehicleType`, no esta tabla — se conecta en la Fase 3. El admin ya puede crear/editar/dar de baja tipos, pero no impacta el sitio en vivo hasta la próxima fase
- [x] `ADMIN_SELECTABLE_ICONS` (whitelist fijo en `constants.ts`) — nunca se importa un ícono por string dinámicamente, el admin elige de un desplegable contra ese whitelist
- [x] Nuevo módulo de permisos `vehiculos` (y `ubicacion`, adelantado para la Fase 5) en `admin-permissions.ts` — mismo criterio que suscripciones/destacados (catálogo reversible, no como identidad): LECTOR solo lee, EDITOR lee+edita, SUPERADMIN además borra
- [x] `/admin/vehiculos`: tabla de Tipos + modal crear/editar (`VehicleTypeFormModal`) + dar de baja/reactivar (`VehicleTypeActiveToggle`) — mismo patrón que Planes en Suscripciones
- [x] Auditado (`vehicleType.create`/`.update`/`.toggleActive`, tabla `VehicleTypeCatalog` sumada a `resolveAuditTargets`/`admin-audit-labels.ts`)
- [x] Sidebar admin actualizado (`AdminSidebarNav.tsx` y el `MOBILE_NAV` duplicado de `AdminHeader.tsx`)
- [x] `tsc --noEmit`, `eslint`, `npm run build` limpios
- [x] Verificado con script desechable contra la base real (crear/editar/dar de baja/reactivar/código duplicado rechazado/borrar, conteo final igual al inicial) y servidor de desarrollo (`/admin/vehiculos` redirige a `/login` sin sesión)
- [ ] Prueba manual en navegador

## Fase 72 — Catálogo administrable (Ubicación + Vehículos): Fase 1/10, refactor de calentamiento (solicitado por el usuario)
Primera fase de un proyecto grande, planificado en modo plan y aprobado explícitamente — ver plan completo en la conversación (10 fases: catálogo de tipos/marcas/modelos/versiones administrable, provincias/localidades administrables, estado "pendiente de aprobación" con descuento de cupo diferido, modales de "no está en la lista" en el wizard, colas de tareas pendientes en el admin, botón "Validar datos"). Esta fase es puramente el refactor base, sin tocar el schema, para que las fases siguientes (en particular "Validar datos" del admin) puedan reusar la misma lógica que ya usa el dueño.
- [x] `buildActivationEffect()` (nuevo, `server/data/listings.ts`) extrae la lógica de "pasar a ACTIVE + consumir cupo" que estaba triplicada en `createListing`/`updateOwnedListing`/`reactivateListing` — refactor puro, sin cambio de comportamiento
- [x] Las 3 funciones existentes reescritas para usar el helper — mismo resultado exacto, código muchísimo más corto
- [x] `tsc --noEmit`, `eslint`, `npm run build` limpios
- [x] Verificado con script desechable contra la base real (pausar→reactivar una publicación real, confirmar que el cupo no se toca para PAUSADA, que `expiresAt` se renueva y `publishedAt` no cambia — igual que antes del refactor) — revertido
- [ ] Prueba manual en navegador
...
- [x] Aclarado: reactivar (`adminSetListingStatus`) nunca chequeó ni consumió cupo — es intencional (excepción administrativa), pero antes pasaba en silencio. Ahora, si el dueño no tiene publicaciones disponibles, el modal de "Marcar Activa" lo dice explícitamente antes de confirmar
- [x] Nueva sección "Historial de auditoría de esta publicación" al pie de `/admin/publicaciones/[id]` — reusa `listAuditLog`/`AuditChangesModal`, filtrado por `targetTable: "Listing", targetId: id` (nuevo filtro `targetId` agregado a `listAuditLog`)
- [x] Diccionario de acciones (`ACTION_LABEL`) movido a `src/lib/admin-audit-labels.ts`, compartido entre `/admin/auditoria` y el historial por publicación — se agrega `listing.pause`
- [x] `tsc --noEmit`, `eslint`, `npm run build` limpios
- [x] Verificado el filtro `targetId` con lectura directa contra la base real
- [ ] Prueba manual en navegador
