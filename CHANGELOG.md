...
- Pendiente (documentado, no bloqueante): cargar `MERCADOPAGO_WEBHOOK_SECRET` desde el panel de Mercado Pago, y probar la confirmación real de un pago de punta a punta (necesita un despliegue o un túnel — Mercado Pago no puede notificar a un `localhost`) con un comprador de prueba.
