...
- `tsc --noEmit`, `eslint`, `npm run build` limpios. Filtros verificados con lectura directa contra la base real.

### Added (2026-08-14) — Cupo y suscripción: nombre del plan visible, contadores de vouchers otorgados/usados, botón Guardar
Reportado en vivo por el usuario: al otorgar una suscripción "30 publicaciones por 30 días" desde `/admin/usuarios/[id]`, "Publicaciones compradas" no se movía y parecía que el otorgamiento no había hecho nada. En realidad sí impactaba — son dos pozos de cupo distintos (`purchasedPublications`, permanente, vs. `subscriptionQuota`, temporal) que se combinan en un único total ("Publicaciones disponibles" = 10 gratis + comprado + suscripción vigente − consumido) — pero el panel no mostraba ese total combinado en ningún lado, así que no había forma de confirmar el efecto real a simple vista.

- **`User.subscriptionPlanCode`** (nuevo): la tarjeta "Suscripción" (admin y `/dashboard/anuncios`) ahora muestra el nombre del plan vigente ("30 publicaciones por 30 días — vence...") en vez de un número de cupo que no bajaba de forma independiente y llevaba a pensar que estaba "trabado".
- **`User.featuredVouchersGranted`/`featuredVouchersUsed`** (nuevos, contadores históricos que nunca decrecen, a diferencia del saldo vivo `pendingFeaturedVouchers`): se incrementan en paralelo en cada sitio donde se otorga un voucher (combo "para la próxima publicación", ajuste manual del admin) o se consume uno (`createListing`/`updateOwnedListing`/`reactivateListing`).
- **`/admin/usuarios/[id]`** y **`/dashboard/anuncios`**: bloque de cupo reordenado con subtítulos explícitos — Suscripción / Publicaciones compradas / Publicaciones realizadas / **Publicaciones disponibles** (destacado, es el número que realmente importa) / Vouchers de destacado compradas / utilizados / disponibles — el usuario ya veía "disponibles" decrementar solo antes de este cambio; ahora también ve el desglose completo, igual que el admin.
- Los botones `+1`/`-1` de "Publicaciones compradas" y "Vouchers de destacado" en el admin (aplicaban al toque, sin confirmación) se reemplazan por un campo de ajuste + botón "Guardar".
- `tsc --noEmit`, `eslint`, `npm run build` limpios. Migración aditiva (3 columnas nuevas en `User`) aplicada contra la base real, con backfill best-effort para `featuredVouchersGranted` (no hay forma de reconstruir el historial exacto anterior a este contador). Verificado con scripts desechables contra la base real: otorgar `SUBSCRIPTION_30` sube "disponibles" en exactamente 30 y resuelve el nombre del plan correctamente; un ajuste de +3/-1 vouchers deja `granted`/`used`/`pending` consistentes — todo revertido al terminar.

### Added (2026-08-14) — Suscripciones y Pagos: editar plan, aprobar pago en efectivo, comprobante desde admin
- **Editar un plan** (`EditPlanModal`, junto a Dar de baja/Reactivar en `/admin/suscripciones`): nombre, descripción, precio, duración en días y cantidad — no toca `code` (rompería el vínculo con `Payment.planCode`, un string suelto sin FK) ni `isActive` (sigue siendo trabajo del toggle existente). El impacto es directo porque `/dashboard/compra` lee `Plan` sin ninguna caché de Next: el cambio se ve en el próximo request.
- Confirmado sin necesidad de tocar código: dar de baja/reactivar/editar un plan ya impactaba directo en las secciones públicas de compra, y reactivar una promoción ya usa el mismo template de tarjeta que sus hermanas — no existen tipos de plan ni componentes de tarjeta separados en el código (el "tipo" es solo una convención de prefijo en `code`).
- **Aprobar un pago en efectivo**: fila `PENDING` en la tabla de Pagos ahora tiene "Aprobar (efectivo)" (motivo obligatorio, `ReasonConfirmModal`) — reusa `applyPaymentEffect()`, la misma función que dispara el webhook de Mercado Pago al confirmar un pago real, en vez de solo pisar `status`. Así un pago aprobado a mano acredita de verdad cupo/suscripción/destacado, nunca queda "aprobado" sin haber entregado lo pagado.
- **Comprobante desde el admin**: fila `APPROVED` tiene "Ver comprobante", reusando el mismo `ComprobanteButton` que ya ve el propio comprador en su Historial de pagos (sin duplicar el componente).
- Gap corregido de paso: `ComprobanteButton` mostraba "Medio de pago: Mercado Pago" fijo incluso en pagos otorgados por un admin (`provider: "admin"`) — ahora se calcula desde el `provider` real del pago (Mercado Pago / Efectivo / Otorgado por administración).
- `tsc --noEmit`, `eslint`, `npm run build` limpios. Verificado con script desechable contra la base real (edición de plan revertida; un `Payment` de prueba PENDING pasado a APPROVED con el mismo efecto que aplicaría el webhook para un pack de publicaciones, cupo acreditado y luego revertido/borrado, sin tocar ninguno de los pagos reales pendientes) — no se pudo probar el flujo completo por navegador en esta sesión por no tener a mano la contraseña de la cuenta SUPERADMIN.
...
- **Identidad**: se reemplaza "Rechazar" (definitivo) por "Guardar observación" — la solicitud sigue `PENDING` en la cola, la nota queda guardada para la próxima revisión en vez de cerrar el caso. Nueva columna "Identidad" (Validada/Pendiente/Sin solicitud) en la lista de `/admin/usuarios`.
- Corregido de paso: `visibleStatusWhere()` (`server/data/listings.ts`) tenía una colisión de claves `OR` con `effectivelyFeaturedWhere()` al combinarse en `getCatalogResults`/`getFeaturedListings` — la última se pisaba a la otra silenciosamente, perdiendo el filtro de estado/vencimiento. Se reescribió para que todo viva dentro de un único `AND`, sin clave `OR` de nivel superior que pueda colisionar.
- Gap detectado mientras se probaba en vivo: bonificar publicaciones/destacados manualmente (`+1`/`-1` en "Suscripción y cupo") no dejaba ningún rastro en el historial de pagos del propio usuario — ahora, un ajuste positivo crea un `Payment` en `$0` (`provider: "admin"`) con la leyenda "otorgado por el administrador", igual que ya hacía `grantSubscription`.
- `tsc --noEmit`, `eslint`, `npm run build` limpios. Migración aditiva (`User`/`Listing.suspendedUntil`/`suspensionReason`, `VerificationRequest.adminNote`, enum `ListingStatus.SUSPENDIDA`) aplicada contra la base real. Verificado con script desechable (suspensión de cuenta oculta sus publicaciones del catálogo pero no de su propio panel, suspender no banea el login, observación de identidad no cambia el status, bonificación deja `Payment`) y con el servidor de desarrollo con login real (columna Identidad, página de Identidad, estado de cuenta en `/dashboard/perfil/tipo-cuenta`).
